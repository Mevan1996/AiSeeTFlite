...
public class CameraSource {
    ...
    public Classifier imageDetector;
    ...
    private final boolean requestedAutoFocus = false;
    ...
    private Thread barcodeDetectionThread;
    private final FrameProcessingRunnable processingRunnable;
    private final BarcodeProcessingRunnabale barcodeProcessingRunnabale;
    ...
    public CameraSource(Activity activity, GraphicOverlay overlay) {
        ...
        barcodeProcessingRunnabale =new BarcodeProcessingRunnabale();
    ...
    public synchronized CameraSource start() throws IOException {
        ...
        barcodeProcessingRunnabale.setActive(true);
        threadPoolExecutor.execute(processingRunnable);
        threadPoolExecutor.execute(barcodeProcessingRunnabale);
        return this;
    }
...
//            processingThread = null;
    ...
    public void setFocus(RectF location){
        Log.e("CenteredorNot",previewSize.getWidth()+" "+previewSize.getHeight());
        final Rect scaledLocation = new Rect (
                ((int) location.left) * 2000/previewSize.getHeight() - 1000,
                ((int) location.top) * 2000/previewSize.getWidth() - 1000,
                ((int)location.right) * 2000/previewSize.getHeight() - 1000,
                ((int) location.bottom )* 2000/previewSize.getWidth() - 1000);
        Log.e("Okayyy", location.left+" "
                + location.top+" "
                +location.right+" "
                +location.bottom);
        Log.e("Okayyy",(((int) location.left) * 2000/previewSize.getHeight() - 1000)+" "
                +(((int) location.top) * 2000/previewSize.getWidth() - 1000)+" "
                +(((int)location.right) * 2000/previewSize.getHeight() - 1000)+" "
                +(((int) location.bottom )* 2000/previewSize.getWidth() - 1000));
        ...
        focusAreas.add(new Camera.Area(scaledLocation ,1000));
        try{
            int maxFocus = camera.getParameters().getMaxNumFocusAreas();
            Log.e("MaximumFocus", "Maximum number of focus areas is"+ maxFocus);
            Camera.Parameters params = camera.getParameters();
            params.setFocusAreas(focusAreas);
            params.setFocusMode(Camera.Parameters.FOCUS_MODE_AUTO);
            Log.e("Focus","Focus set successfully pre");
            ...
            Log.e("Focus","Focus set successfully");
        }
        catch (Exception e){
            Log.e("FocusException","Error while trying to set focus");
        }
    ...
    private class CameraPreviewCallback implements Camera.PreviewCallback {
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            ...
            barcodeProcessingRunnabale.setNextFrame(data,camera);
        }
    }
